...
# 📊 Automatización de Reportes Epidemiológicos: UC-IRAG Catamarca
...
* **Perfil Clínico y Epidemiológico:** Describir la distribución de los casos de IRAG e IRAGe según grupo etario, sexo y diagnóstico de ingreso (*Bronquiolitis, Neumonía, Sepsis, Shock u Otros*).
...
* `Plan de análisis UC IRAG HINEP CATAMARCA.pdf`: Plan de análisis metodológico y mapa del circuito de datos.  
...
> [!WARNING]
> **IMPORTANTE:** Con el fin de garantizar el secreto profesional y cumplir con las normativas vigentes sobre la protección de datos personales y médicos, las bases de datos nominales y agrupadas utilizadas para este análisis **no se comparten de manera pública** en este repositorio.

Para reproducir localmente los elementos de visualización (gráficos de *Highcharter/ggplot2*, tablas de resumen y mapas del circuito), el usuario institucional deberá incorporar de forma local las siguientes fuentes:
...
```r
install.packages(c("tidyverse", "dplyr", "ggplot2", "lubridate", "stringr", 
...
1. **Clonar** o descargar este repositorio localmente.
2. Abrir el proyecto `.Rproj` e **incluir las bases de datos** protegidas en la ruta local correspondiente.
3. **Renderizar** el archivo `.qmd` para compilar el informe epidemiológico automatizado.
