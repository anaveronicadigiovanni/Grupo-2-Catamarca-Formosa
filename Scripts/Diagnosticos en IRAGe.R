#####Diagnosticos en IRAGe, HAgo tabla###

tabla_DX_IRAGe <- data_irage |>
...
tabla_grafico_DX2 <- tabla_DX_IRAGe %>%
...
tabla_DX_IRAGe <- tabla_grafico_DX2 %>%
  gt() %>%
  
  # Títulos y subtítulos 
  tab_header(
    title = "Diagnósticos al ingreso en IRAGe"
  ) %>%
...
# # 1. Limpieza y filtrado
# tabla_grafico_DX_irage <- tabla_DX_IRAGe %>%
#   mutate(Diagnóstico = str_replace_all(Diagnóstico, "_", " "),
#          Diagnóstico = str_to_sentence(Diagnóstico)) %>%
...
#   hc_subtitle(text = "") %>%
#   hc_xAxis(categories = tabla_grafico_DX_irage$Diagnóstico,
#            title = list(text = NULL)) %>% 
#   hc_yAxis(title = list(text = "Prevalencia (%)"),
#            labels = list(format = "{value}%"),
#            max = 100) %>%
#   hc_add_series(
#     data = tabla_grafico_DX_irage$Prevalencia,
#     name = "Porcentaje",
#     color = "#2c3e90", # azul 
#     showInLegend = FALSE
#   ) %>%
