...
tabla_comorb_irag <- tabla_comorbilidades_IRAG %>%
  mutate(
...
tabla_comorb_IRAG <- tabla_comorb_irag %>%
  gt() %>%
  
  # Títulos y subtítulos 
  tab_header(
    title = "Frecuencia de las comorbilidades en IRAG"
  ) %>%
...
###hago tabla de comorbilidades en las IRAG CON PORCENTAJES####
# data_irag_com<-data_irag |> filter(PRESENCIA_COMORBILIDADES_texto=="Con comorbilidades")
# 
# tabla_comorbilidades_IRAG <- data_irag |>
# 
#   summarise(across(c(DIABETES , BAJO_PESO_NACIMIENTO  ,  ASMA , TUBERCULOSIS,                       
#                      PREMATURIDAD,
#                      ENF_RESPIRATORIA , CARDIOPATIA_CONGENITA  , VIH , ASPLENIA ,                            
#                      DESNUTRICION , CANCER ,  TRASPLANTADO  , BRONQUIOLITIS_PREVIA,                                      
#                      ENF_NEUROLOGICA_CRONICA , ENF_HEPATICA , HIPERTENSION ,                   
#                      ENF_CEREBROVASCULAR  , ENF_NEUROMUSCULAR , DISCAPACIDAD_INTELECTUAL ,          
#                      ENF_CARDIACA  , ENF_REUMATOLOGICA  , DBP , ENF_RENAL  , OBESIDAD                             
#                      ,S_DOWN,INMUNOCOMPROMETIDO_OTRAS_CAUSAS , OTRAS_COMORBILIDADES ), 
...
#          Comorbilidades = str_to_sentence(Comorbilidades)) %>%
#   filter(Frecuencia > 0) # Filtramos las que no tienen casos
# 
...
# tabla_IRAG_com<-highchart() %>%
#   hc_chart(type = "bar") %>% 
#   hc_title(text = "Prevalencia de Comorbilidades en Casos de IRAG") %>%
